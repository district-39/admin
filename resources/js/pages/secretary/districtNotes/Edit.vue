...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: NoteController.edit.url(props.note.id) },
];

const form = useForm({
    district_meeting_id: props.note.district_meeting_id ?? ('' as string | number | null),
    file_id: props.note.file_id,
    attendees: props.note.attendees.map((a) => ({
        user_id: a.user_id,
        email: a.email ?? '',
...
function submit(): void {
    form.put(NoteController.update.url(props.note.id), {
        preserveScroll: true,
    });
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full max-w-2xl space-y-6 p-6">
            <Heading title="Edit Note" :description="`Update note #${note.id}`" />
